...
# ── Configure this path ────────────────────────────────────────────────────
FEATURES_DIR = "/Users/hamishdawson/Desktop/Thesis/Thesis-Auslan-Translator/gloss_clips_features"
# ──────────────────────────────────────────────────────────────────────────
...
def main():
    ...
    for gloss_dir in sorted(features_dir.iterdir()):
        if not gloss_dir.is_dir():
            continue
        orig = len([p for p in gloss_dir.glob("*.npy")
                    if p.stem.split("_")[-1] not in suffixes])
        total = len(list(gloss_dir.glob("*.npy")))
        print(f"  {gloss_dir.name:<16} {orig} → {total}")
